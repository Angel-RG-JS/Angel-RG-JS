# 👋 Hi, I’m @Angel-RG-JS

### 🚀 Backend Developer | International Business @ IPN
*Focusing on how predictive systems and web architecture drive global business expansion.*

---

### 🛠️ My Tech Profile
```javascript
const angel = {
  main_stack: ["Java", "Spring Boot", "Predictive Engines"],
  domain: "Fintech & Business Intelligence",
  background: "International Business Management (IPN 🇲🇽)",
  current_project: "FIRE Predictive Engine",
  dogs_lover: true
};
```

🌟 Featured Project

FIRE Predictive Engine A specialized engine designed for financial forecasting and predictive modeling.

    Core: Java-based backend logic.

    Focus: Data-driven decision making for financial independence and business growth.

🎯 What I'm working on

    🧠 Refining predictive algorithms for financial models.

    🌐 Exploring scalable cloud infrastructure for backend services.

    📈 Merging IPN business logic with robust software engineering.

📫 Connect with me

    📧 Email: angelrangel.2392@gmail.com

    📍 Based in: Mexico City
